...
{{TimeOfDay == Dawn or TimeOfDay == Night:You try the door which, thankfully, turns out to be unlocked. It opens with a slight creak and| The door} gives way to a long, L-shaped room, stretching some 40 yards back and 15 across. In the L's hook, to the right of the entrance, an ashwood bar stands worn but well kept. {RubertLocations == RuLEdanInnCommonRoom:Behind it, {knows(RobertKnowState.Name):Robert|a burly man} is cleaning a mug. He nods at you in welcome as you enter.} The rest of the room is filled with all manner of tables and chairs, made from a variety of materials such as metal, wood and stone{TimeOfDay >= Morning and TimeOfDay <= Dusk:, which are occupied by a few patrons. {knows(EdieKnowState.Name):Adelaine|A servant girl} is busily cleaning an empty table in the back}{TimeOfDay == Evening:, which are almost all occupied by patrons. {knows(EdieKnowState.Name):Adelaine|A servant girl} is busily running to and fro to bring people their orders}. In the center of the left wall a hearth is burning, providing the room with warmth and a flickering of orange light{Weather >= LightRain:, offsetting the clatter of rain against the windows}. {TimeOfDay == Morning:The morning sun beams through the windows that are set into the right wall}{TimeOfDay == Midday and Weather <= LightClouds:Sunlight pours in through the windows that are set into the front and right sides of the room}{TimeOfDay == Dusk and Weather <= LightClouds:The Twilight gleam seeps in through the windows that are set into the front side of the building}{TimeOfDay == Evening:Lit chandeliers hang from the suprisingly high ceiling, giving the room some extra light. }{TimeOfDay == Dusk or TimeOfDay == Evening and AliceLocations == AlLEdanInnCommonRoom:{TimeOfDay == Dusk:The conversations between the few early visitors|the patrons' banter} is accompanied by the gentle strum of a guitar. Its source is found in the back corner: {knows(AliceKnowState.Name):Alice|a young woman with goldenbrown hair} is seated on a stool, playing her instrument}{TimeOfDay == Dawn or TimeOfDay == Night:{knows(EdieKnowState.Name):Adelaine|A girl} sits crouched in front of the fire, stoking it with a poker. As you enter, she looks up at you in surprise. -> EdanInnEntranceConvoEdie}.|You step into the Edani Inn. The hearth is burning merrily{TimeOfDay == Midday and Weather <= LightClouds:, even though the sun is at its zenith,}{TimeOfDay >= Morning and TimeOfDay <= Dusk: and a few patrons are having a drink or bite to eat.}{TimeOfDay == Evening:, to the joy of the many patrons packed into the place.}{TimeOfDay == Night or TimeOfDay == Dawn and EdieLocations == EdLEdanInnCommonRoom:, undoubtedly kept going by {knows(EdieKnowState.Name):Adelaide,|the servant girl} who's currently tending to it.}}

->EdanInnCommonRoomChoices
=EdanInnEntranceConvoEdie
<BR>
{knows(EdieKnowState.Name):"Oh, it's you {PlayerName}! Sorry, I wasn't expecting anyone at this hour{AffEdie >= 50: but please, do come in!|.}"|"Oh, I'm sorry Sir/Madam/They, I wasn't expecting anyone at this hour."}
//Missing a sir/madam option in the pronouns. 
->EdanInnCommonRoomChoices

=EdanInnCommonRoomChoices
+[Look at...]
    ++{AliceLocations == AlLEdanInnCommonRoom} \ [{knows(AliceKnowState.Name) == 1:Alice|The bard}]
    ->EdanInnCommonRoomConvoAlice
    ++{RubertLocations == RuLEdanInnCommonRoom} \ [{knows(RobertKnowState.Name):Rubert|The bartender}]
    ->EdanInnCommonRoomChoices
    ++{EdieLocations == EdLEdanInnCommonRoom} \ [{knows(EdieKnowState.Name):Adelaine|The servant girl}]
...
+[Talk to...]
    ++{AliceLocations == AlLEdanInnCommonRoom} \ [{knows(AliceKnowState.Name) == 1:Alice|The bard}]
    ->EdanInnCommonRoomChoices
    ++{RubertLocations == RuLEdanInnCommonRoom} \ [{knows(RobertKnowState.Name):Robert|The bartender}]
...
+{AliceLocations == AlLEdanInnCommonRoom} \ [{knows(AliceKnowState.Name) == 1:Alice|The bard}]
->EdanInnCommonRoomLookAt
+{RubertLocations == RuLEdanInnCommonRoom} \ [{knows(RobertKnowState.Name) == 1:Robert|The bartender}]
->EdanInnCommonRoomLookAt
+{EdieLocations == EdLEdanInnCommonRoom} \ [{knows(EdieKnowState.Name):Adelaine|The servant girl}]
->EdanInnCommonRoomLookAt
+[Actually, nevermind.] 
